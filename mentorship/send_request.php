...
try {
    $db = getDB();
    $alumni = $db->fetchAll(
        "SELECT account_id, name, education, skills, location, bio 
         FROM accounts 
    ...
    if (!empty($alumni_id)) {
        try {
            $selectedAlumni = $db->fetchOne(
                "SELECT account_id, name FROM accounts WHERE account_id = ? AND role = ? AND is_active = TRUE",
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="alumni_id" name="alumni_id" required>
                            ...
                            <?php foreach ($alumni as $alum): ?>
                                <option value="<?php echo $alum['account_id']; ?>" <?php echo (isset($_POST['alumni_id']) && $_POST['alumni_id'] == $alum['account_id']) ? 'selected' : ''; ?>>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        alumniSelect.addEventListener('change', function () {
            ...
            if (selectedId) {
                const selectedAlumni = alumniData.find(alum => alum.account_id === selectedId);
