...
if ($user_role === ROLE_ALUMNI) {
    ...
    $pageTitle = 'My Mentorships';
    $pendingRequests = []; // Students don't have pending requests to review
}
...
try {
    if ($user_role === ROLE_ALUMNI) {
        $activeMentorships = $db->fetchAll(
            "SELECT m.*, s.name as partner_name, s.email as partner_email,
        ...
        // Student view - show their mentorships with alumni
        $activeMentorships = $db->fetchAll(
            "SELECT m.*, a.name as partner_name, a.email as partner_email,
                    a.education, a.skills, a.location,
                    (SELECT COUNT(*) FROM messages WHERE mentorship_id = m.mentorship_id AND is_read = FALSE AND receiver_id = ?) as unread_count,
                    (SELECT MAX(sent_at) FROM messages WHERE mentorship_id = m.mentorship_id) as last_message_at
             FROM mentorships m
...
// Get mentorship history based on user role
try {
    if ($user_role === ROLE_ALUMNI) {
        $mentorshipHistory = $db->fetchAll(
            "SELECT mr.*, s.name as partner_name, s.education
             FROM mentorship_requests mr
             JOIN accounts s ON mr.student_id = s.account_id
             WHERE mr.alumni_id = ? AND mr.status IN (?, ?)
             ORDER BY mr.responded_at DESC
             LIMIT 10",
            [$user_id, MENTOR_STATUS_ACCEPTED, MENTOR_STATUS_DECLINED]
        );
    } else {
        // Student view - show their request history
        $mentorshipHistory = $db->fetchAll(
            "SELECT mr.*, a.name as partner_name, a.education
             FROM mentorship_requests mr
             JOIN accounts a ON mr.alumni_id = a.account_id
             WHERE mr.student_id = ? AND mr.status IN (?, ?)
             ORDER BY mr.responded_at DESC
             LIMIT 10",
            [$user_id, MENTOR_STATUS_ACCEPTED, MENTOR_STATUS_DECLINED]
        );
                            ...
                            <div class="modal fade" id="mentorshipModal<?php echo $mentorship['mentorship_id']; ?>"
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title">
                                                Mentorship with <?php echo sanitizeInput($mentorship['partner_name']); ?>
                                            </h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <h6><?php echo $user_role === ROLE_ALUMNI ? 'Student' : 'Alumni'; ?>
                                                        Information</h6>
                                                    <p><strong>Name:</strong>
                                                        <?php echo sanitizeInput($mentorship['partner_name']); ?></p>
                                                    <p><strong>Email:</strong>
                                                        <?php echo sanitizeInput($mentorship['partner_email']); ?></p>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-sm">
                            <thead>
                                <tr>
                                    <th><?php echo $user_role === ROLE_ALUMNI ? 'Student' : 'Alumni'; ?></th>
                                    <th>Education</th>
                                    <th>Response</th>
                                    <th>Date</th>
                                </tr>
                            ...
                            <tbody>
                                <?php foreach ($mentorshipHistory as $history): ?>
                                    <tr>
                                        <td><?php echo sanitizeInput($history['partner_name']); ?></td>
                                        <td>
