...
/**
 * Log security events with fallback mechanisms
 */
function logSecurityEvent($event, $details = '', $severity = 'INFO')
{
    ...
    if (!$logged) {
        $tempLogFile = sys_get_temp_dir() . '/nyumba_security.log';
    ...
    if (!$logged) {
        error_log("SECURITY LOG: " . trim($logEntry));
        $logged = true;
    }

    // If all else fails, store in session for later (last resort)
    if (!$logged) {
        if (!isset($_SESSION['pending_security_logs'])) {
            $_SESSION['pending_security_logs'] = [];
        }
        $_SESSION['pending_security_logs'][] = $logEntry;
    }
}
