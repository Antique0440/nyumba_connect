    ...
    if (!checkCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                $db = getDB();
                $user = $db->fetchOne(
                    "SELECT account_id, name, email, password_hash, role, is_active FROM accounts WHERE email = ?",
                    [$email]
                );
                ...
                if ($user) {
                    ...
                    if ($passwordCheck['valid']) {
                        ...
                        if (!$user['is_active']) {
                            ...
                            if ($passwordCheck['needs_rehash']) {
                                try {
                                    $db->execute(
                                        ...
                                        [$passwordCheck['new_hash'], $user['account_id']]
                                    );
                                } catch (Exception $e) {
                            ...
                            logActivity($user['account_id'], 'Login', "IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
                    ...
                    if ($user) {
                        logActivity($user['account_id'], 'Failed Login', "IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
                    }
                }
            } catch (Exception $e) {
