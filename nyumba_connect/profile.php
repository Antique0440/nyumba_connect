        ...
        if ($profile_user_id !== $user_id && !isAdmin()) {
            ...
            if (empty($errors)) {
                try {
                    ...
                    $db->execute(
                        "UPDATE users SET name = ?, email = ?, year_left = ?, education = ?, skills = ?, location = ?, bio = ? WHERE user_id = ?",
                        [
                            $name,
                            $email,
                            $year_left ?: null,
...
try {
    ...
    $profile_user = $db->fetchOne(
        "SELECT user_id, name, email, role, year_left, education, skills, location, bio, cv_path, is_active, created_at FROM users WHERE user_id = ?",
        [$profile_user_id]
    );
                    ...
                    <div id="profileView">
                        ...
                        <?php if ($profile_user['cv_path'] && file_exists($profile_user['cv_path'])): ?>
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <a href="<?php echo SITE_URL; ?>/download_cv.php?user_id=<?php echo $profile_user['user_id']; ?>"
